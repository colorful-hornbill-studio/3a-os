<?php
    if(isset($_GET["path"]) && is_dir($_GET["path"])) {
        ...
        for($z = 0; $z < count($files); ++$z) {
            if(pathinfo($files[$z], PATHINFO_EXTENSION) === "mp4" || pathinfo($files[$z], PATHINFO_EXTENSION) === "webm" || pathinfo($files[$z], PATHINFO_EXTENSION) === "ogg") {
                echo "<a class='list' onclick='select(this.id)' title='file' id='" . 
                    htmlspecialchars(pathinfo($files[$z], PATHINFO_BASENAME)) . "-is-not-element'><img src='/3a-os/pictures/komponen-web/video-file.svg' " . 
