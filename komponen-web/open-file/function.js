...
function openFileAndFolder() {
    if(selectedFile.title === "file") {
        appRequest += "?directory=../../storage/" + path1 + "&file_name=" + name;
        location.assign(appRequest);
...
function refresh() {
    ...
    if(type === "") {
        request = "/3a-os/komponen-web/file-operations/read-dir.php?path=../../storage/" + path1;
    } else if(type === "image") {
        request = "/3a-os/komponen-web/file-operations/read-dir-for-image.php?path=../../storage/" + path1;
    } else if(type === "video") {
        request = "/3a-os/komponen-web/file-operations/read-dir-for-video.php?path=../../storage/" + path1;
    }
...
function setFileManager(appPath, tempType) {
    ...
    ajax4.onreadystatechange = function() {
        if(this.readyState === 4 && this.status === 200) {
            infoFile = "<div>File Name: <br/>&nbsp;" + name + "</div><div>Extension: <br/>&nbsp;" + this.responseText + "</div><div> Location: <br/>&nbsp;" + path1 + "</div>";
    ...
    ajax5.onreadystatechange = function() {
        if(this.readyState === 4 && this.status === 200) {
            infoFile = "<div>Folder Name: <br/>&nbsp;" + name + "</div><div>Folder Size: <br/>&nbsp;" + this.responseText + "</div><div> Location: <br/>&nbsp;" + path1 + "</div>";
